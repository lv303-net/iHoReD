IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'GET_GENERAL_SALARY_STATISTICS_FOR_PERIOD')
	DROP PROC GET_GENERAL_SALARY_STATISTICS_FOR_PERIOD
GO
CREATE PROCEDURE GET_GENERAL_SALARY_STATISTICS_FOR_PERIOD
	@DOCTOR_ID INT,
	@START_DATE DATETIME,
	@END_DATE DATETIME
AS
BEGIN
	...
	END;
	INSERT INTO @TEMP VALUES (NULL, (SELECT SUM(WORKED_HOURS) FROM @TEMP), (SELECT AVG(RATE) FROM @TEMP), (SELECT AVG(COEFFICIENT) FROM @TEMP), (SELECT SUM(TOTAL) FROM @TEMP))
	SELECT * FROM @TEMP WHERE DAYY IS NULL
END;
GO
