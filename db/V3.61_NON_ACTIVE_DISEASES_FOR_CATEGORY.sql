...
AS
BEGIN
    SELECT * FROM ICATEGORIES INNER JOIN SUBCATEGORIES ON CATEGORYCODE=CATEGORYID INNER JOIN DISEASES ON SUBCATEGORIEID=SUBCATEGORYID
    ...
    OR (ID_DISEASE IS NULL OR PATIENT_DISEASES.END_DATETIME<=GETDATE()) AND DISEASES.DISEASEID=@DISEASEID;
END;
