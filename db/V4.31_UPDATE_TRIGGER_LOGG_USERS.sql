    ...
    BEGIN
		...
		BEGIN
			SET @QUERY  = N'SELECT DELETED.'+ @COL_NAME + ',' + 'INSERTED.' + @COL_NAME +' 
				 ...
				 RIGHT JOIN #INSERTED INSERTED ON Inserted.IDUSERS = DELETED.IDUSERS
...
PRINT @NEW;
PRINT @FIRST_COLUMN;
INSERT INTO LOGGER(OLD, NEW, COLUMN_NAME, TABLE_NAME, DATE_OF_UPDATE, USER_ID, ROW_ID) VALUES(@OLD, @NEW, @COL_NAME, 'USERS', GETDATE(), @USER, @ROW_ID);
END 
  END
...
DROP TABLE #DELETED 
DROP TABLE #INSERTED
SELECT * INTO ##USER FROM USER_UPDATE 
