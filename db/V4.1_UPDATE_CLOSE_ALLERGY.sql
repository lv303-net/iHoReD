...
BEGIN
    ...
    WHERE IDPATIENT=@ID_USER AND ALLERGY_ID=@ID_ALLERGY AND START_DATETIME=@START_VISIT_TIME AND USERS_ALLERGIES.END_DATETIME IS NOT NULL)
    RETURN 0;
    ELSE 
 BEGIN
    BEGIN TRANSACTION;
    UPDATE USERS_ALLERGIES 
    SET END_DATETIME=(SELECT SCHEDULE.ID FROM SCHEDULE WHERE START_DATETIME=@START_VISIT_TIME AND IDPATIENT=@ID_USER AND USERS_ALLERGIES.END_DATETIME IS NULL)
    ...
    WHERE IDPATIENT=@ID_USER AND ALLERGY_ID=@ID_ALLERGY AND START_DATETIME=@START_VISIT_TIME AND USERS_ALLERGIES.END_DATETIME IS NULL);
		COMMIT TRANSACTION;
		RETURN 1;
	END;
	END;
